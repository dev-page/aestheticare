...
const selectedPlan = ref(String(route.query.plan || 'free-trial').trim().toLowerCase())
const paymentId = ref(String(route.query.paymentId || '').trim())
const paymentStatus = ref(String(route.query.paymentStatus || '').trim().toLowerCase())
...
const documentUploadState = ref({
  ...
  prcLicenseMedicalDirector: { uploading: false, error: '' },
})
...
const selectedPlanLabel = computed(() => {
  const raw = String(selectedPlan.value || '').trim().toLowerCase()
...
const hasDocumentChanges = computed(() => (
  Boolean(businessPermitFile.value) ||
  Boolean(dohAccreditationFile.value) ||
  Boolean(fdaApprovalFile.value) ||
  Boolean(prcLicenseFile.value)
))
const canReturnToWaitingWithoutResubmit = computed(() => pendingApprovalMode.value && !hasDocumentChanges.value)
const documentStepPrimaryLabel = computed(() => {
  if (canReturnToWaitingWithoutResubmit.value) return 'Back to Waiting for Approval'
  if (pendingApprovalMode.value) return 'Submit Documents Again'
  return 'Submit Documents'
})
const allDocumentsUploaded = computed(() => (
  Boolean(businessPermitFile.value || existingSubmittedDocuments.value.businessPermit) &&
  Boolean(dohAccreditationFile.value || existingSubmittedDocuments.value.dohAccreditation) &&
  Boolean(fdaApprovalFile.value || existingSubmittedDocuments.value.fdaApproval) &&
  Boolean(prcLicenseFile.value || existingSubmittedDocuments.value.prcLicenseMedicalDirector)
))
...
onMounted(() => {
  ...
  const qPlan = String(route.query.plan || '').trim().toLowerCase()
  const qPaymentId = String(route.query.paymentId || '').trim()
  const qPaymentStatus = String(route.query.paymentStatus || '').trim().toLowerCase()

  if (qFirstName && !firstName.value) firstName.value = qFirstName
  if (qLastName && !lastName.value) lastName.value = qLastName
  if (qEmail && !email.value) email.value = qEmail.toLowerCase()
  if (qPlan) selectedPlan.value = qPlan
  if (qPaymentId) paymentId.value = qPaymentId
  if (qPaymentStatus) paymentStatus.value = qPaymentStatus
...
const handleDocumentFileChange = async (key, event) => {
  ...
  if (key === 'businessPermit') businessPermitFile.value = selectedFile
  if (key === 'dohAccreditation') dohAccreditationFile.value = selectedFile
  if (key === 'fdaApproval') fdaApprovalFile.value = selectedFile
  if (key === 'prcLicense') prcLicenseFile.value = selectedFile

  if (selectedFile) {
    if (key === 'businessPermit') existingSubmittedDocuments.value.businessPermit = null
    if (key === 'dohAccreditation') existingSubmittedDocuments.value.dohAccreditation = null
    if (key === 'fdaApproval') existingSubmittedDocuments.value.fdaApproval = null
    if (key === 'prcLicense') existingSubmittedDocuments.value.prcLicenseMedicalDirector = null
  }

  const previewKey = key === 'prcLicense' ? 'prcLicenseMedicalDirector' : key
...
const syncExistingDocumentPreviews = () => {
  ...
  documentPreviewUrls.value.businessPermit = docs.businessPermit?.url && docs.businessPermit?.type?.startsWith('image/')
    ? docs.businessPermit.url
    : ''
  documentPreviewUrls.value.dohAccreditation = docs.dohAccreditation?.url && docs.dohAccreditation?.type?.startsWith('image/')
    ? docs.dohAccreditation.url
    : ''
  documentPreviewUrls.value.fdaApproval = docs.fdaApproval?.url && docs.fdaApproval?.type?.startsWith('image/')
    ? docs.fdaApproval.url
    : ''
  documentPreviewUrls.value.prcLicenseMedicalDirector = docs.prcLicenseMedicalDirector?.url && docs.prcLicenseMedicalDirector?.type?.startsWith('image/')
    ? docs.prcLicenseMedicalDirector.url
    : ''
}
...
const mapRegistrationStepFromStatus = (userStatus, clinicStatus) => {
  ...
  if (normalizedUserStatus === 'active') return 'active'
...
const verifyRegistrationEmail = async () => {
  ...
  try {
    ...
    clinicName.value = clinicData.clinicName || clinicName.value
    clinicLocation.value = clinicData.clinicLocation || clinicLocation.value
    clinicLocationLat.value = clinicData.clinicLocationLat || clinicLocationLat.value
    clinicLocationLng.value = clinicData.clinicLocationLng || clinicLocationLng.value
    clinicLocationAddress.value = clinicData.clinicLocationAddress || clinicLocationAddress.value
    const storedDocuments = clinicData?.submittedDocuments || clinicData?.draftDocuments || {}
    existingSubmittedDocuments.value = {
      ...
      prcLicenseMedicalDirector: storedDocuments?.prcLicenseMedicalDirector || null,
    }
    syncExistingDocumentPreviews()
    ...
    try {
      ...
      if (!paymentsSnap.empty) {
        ...
        if (latestPayment?.planId) {
          selectedPlan.value = latestPayment.planId
          paymentStatus.value = 'paid'
          paymentId.value = latestPayment.id

          const planDays = latestPayment.planId === 'free-trial' ? 14 : 30
          const baseMillis = latestPayment.paidAtSeconds
            ? latestPayment.paidAtSeconds * 1000
            : latestPayment.createdAt
              ? latestPayment.createdAt * 1000
              : Date.now()
          const subscriptionStartedAt = new Date(baseMillis)
          const subscriptionExpiresAt = new Date(baseMillis + planDays * 24 * 60 * 60 * 1000)
      ...
      console.error('Failed to sync payment info:', paymentErr)
    }
...
const handlePlaceSelection = (location, components, fallbackName) => {
  ...
  const isCavite =
    comp.some((c) => c.types?.includes('administrative_area_level_2') && /cavite/i.test(c.longName || c.long_name)) ||
    comp.some((c) => c.types?.includes('administrative_area_level_1') && /calabarzon/i.test(c.longName || c.long_name))
  if (!isCavite) {
  ...
  const cityName = cityComponent?.longName || cityComponent?.long_name
  clinicLocation.value = cityName || fallbackName
  clinicLocationAddress.value = fallbackName || ''
  locationError.value = ''
}
...
const reverseGeocodeLocation = (lat, lng) => {
  ...
  geocoder.geocode({ location: { lat, lng } }, (results, status) => {
    ...
    const isCavite =
      components.some((c) => c.types.includes('administrative_area_level_2') && /cavite/i.test(c.long_name)) ||
      components.some((c) => c.types.includes('administrative_area_level_1') && /calabarzon/i.test(c.long_name))
    if (!isCavite) {
    ...
    clinicLocation.value = cityName || results[0].formatted_address || ''
    clinicLocationLat.value = String(lat)
    clinicLocationLng.value = String(lng)
    clinicLocationAddress.value = results[0].formatted_address || ''
    locationError.value = ''
    closeLocationModal()
  })
}
...
const registerClinic = async () => {
  ...
  if (!caviteLocations.includes(clinicLocation.value)) {
    ...
    if (userUid.value) {
      ...
      await Promise.all([
        updateDoc(doc(db, 'users', userUid.value), {
          ...
          subscriptionPlan: selectedPlan.value || 'free-trial',
...
const handleDocumentStepPrimaryAction = async () => {
  if (canReturnToWaitingWithoutResubmit.value) {
    currentStep.value = 4
    return
  }
...
const submitDocuments = async () => {
  ...
  try {
    const [uploadedBusinessPermit, uploadedDoh, uploadedFda, uploadedPrc] = await Promise.all([
      uploadDocumentForClinic(userUid.value, businessPermitFile.value, 'businessPermit'),
      uploadDocumentForClinic(userUid.value, dohAccreditationFile.value, 'dohAccreditation'),
      uploadDocumentForClinic(userUid.value, fdaApprovalFile.value, 'fdaApproval'),
      uploadDocumentForClinic(userUid.value, prcLicenseFile.value, 'prcLicenseMedicalDirector'),
    ])

    const submittedDocumentsPayload = {
      businessPermit: uploadedBusinessPermit || existingSubmittedDocuments.value.businessPermit || { name: '', size: 0, type: '', url: '' },
      dohAccreditation: uploadedDoh || existingSubmittedDocuments.value.dohAccreditation || { name: '', size: 0, type: '', url: '' },
      fdaApproval: uploadedFda || existingSubmittedDocuments.value.fdaApproval || { name: '', size: 0, type: '', url: '' },
      prcLicenseMedicalDirector: uploadedPrc || existingSubmittedDocuments.value.prcLicenseMedicalDirector || { name: '', size: 0, type: '', url: '' },
    }
...
<template>
  <div class="min-h-[100dvh] lg:h-screen bg-gradient-to-br from-cream-50 via-cream-100 to-gold-100 overflow-x-hidden no-scrollbar relative">
    ...
    <div class="relative z-10 flex items-center justify-center px-4 pt-24 pb-8 lg:pt-20 lg:pb-2 lg:min-h-[calc(100dvh-4rem)] text-sm">
      <div class="register-clinic-card relative w-full max-w-6xl rounded-3xl overflow-hidden bg-white/68 backdrop-blur-xl border border-gold-200/60 shadow-2xl shadow-gold-900/15 lg:h-[calc(100dvh-5rem)]">
        <div class="compact-desktop register-form-panel relative z-10 flex items-center justify-center h-full px-4 pt-8 pb-8 lg:pt-3 lg:pb-3 sm:px-8">
          ...
          <form class="space-y-4 w-full max-w-[560px] pt-3 lg:pt-5" @submit.prevent="registerClinic">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 rounded-2xl border border-gold-200/80 bg-cream-50/80 p-4">
              <div>
                <p class="text-xs text-charcoal-500 mb-1">Selected Plan</p>
                <p class="text-sm font-semibold text-charcoal-700">{{ selectedPlanLabel }}</p>
              </div>
              <div>
                <p class="text-xs text-charcoal-500 mb-1">Payment Status</p>
                <p class="text-sm font-semibold text-charcoal-700">{{ paymentStatusLabel }}</p>
              </div>
            </div>
            ...
            <div class="space-y-2 rounded-2xl border border-gold-200/80 bg-cream-50/80 p-4">
              <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <button
                  v-else
                  type="button"
                  class="h-12 sm:h-14 px-5 rounded-xl border border-gold-300 text-charcoal-700 font-medium hover:bg-gold-100/70 transition"
                  @click="currentStep = 4"
                >
            ...
            <div class="cta-row flex gap-3">
              <button
                type="button"
                class="h-14 lg:h-12 px-5 rounded-xl border border-gold-300 text-charcoal-700 font-medium hover:bg-gold-100/70 transition"
                @click="router.back()"
              >
                Back
              </button>
            ...
            <section v-if="currentStep === 2" class="space-y-5">
            ...
            <section v-if="currentStep === 3" class="space-y-4">
              ...
              <div class="upload-grid">
                <div class="upload-card">
                  <p class="upload-label">Business Permit</p>
                  <input type="file" accept=".pdf,image/*" @change="handleDocumentFileChange('businessPermit', $event)" class="upload-input" />
                  <img v-if="documentPreviewUrls.businessPermit" :src="documentPreviewUrls.businessPermit" alt="Business Permit Preview" class="upload-preview-image" />
                  <p class="upload-file-name">{{ businessPermitFile?.name || existingSubmittedDocuments.businessPermit?.name || 'No file selected' }}</p>
                </div>

                <div class="upload-card">
                  <p class="upload-label">DOH Accreditation</p>
                  <input type="file" accept=".pdf,image/*" @change="handleDocumentFileChange('dohAccreditation', $event)" class="upload-input" />
                  <img v-if="documentPreviewUrls.dohAccreditation" :src="documentPreviewUrls.dohAccreditation" alt="DOH Accreditation Preview" class="upload-preview-image" />
                  <p class="upload-file-name">{{ dohAccreditationFile?.name || existingSubmittedDocuments.dohAccreditation?.name || 'No file selected' }}</p>
                </div>

                <div class="upload-card">
                  <p class="upload-label">FDA Approval</p>
                  <input type="file" accept=".pdf,image/*" @change="handleDocumentFileChange('fdaApproval', $event)" class="upload-input" />
                  <img v-if="documentPreviewUrls.fdaApproval" :src="documentPreviewUrls.fdaApproval" alt="FDA Approval Preview" class="upload-preview-image" />
                  <p class="upload-file-name">{{ fdaApprovalFile?.name || existingSubmittedDocuments.fdaApproval?.name || 'No file selected' }}</p>
                </div>

                <div class="upload-card">
                  <p class="upload-label">PRC License of Medical Director</p>
                  <input type="file" accept=".pdf,image/*" @change="handleDocumentFileChange('prcLicense', $event)" class="upload-input" />
                  <img v-if="documentPreviewUrls.prcLicenseMedicalDirector" :src="documentPreviewUrls.prcLicenseMedicalDirector" alt="PRC License Preview" class="upload-preview-image" />
                  <p class="upload-file-name">{{ prcLicenseFile?.name || existingSubmittedDocuments.prcLicenseMedicalDirector?.name || 'No file selected' }}</p>
                </div>
              </div>
              ...
              <div class="flex gap-3">
                ...
                <button
                  ...
                  @click="handleDocumentStepPrimaryAction"
                >
                  {{ isSubmittingDocuments ? 'Submitting...' : documentStepPrimaryLabel }}
                </button>
              </div>
            </section>

            <section v-if="currentStep === 4" class="space-y-4 rounded-2xl border border-gold-200/80 bg-cream-50/80 p-6">
      ...
      >
        <div class="space-y-4">
          ...
          <p v-if="locationError" class="text-sm text-rose-400">{{ locationError }}</p>
