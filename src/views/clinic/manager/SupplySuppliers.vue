            ...
            <div class="pt-4 border-t border-slate-700">
              ...
              <div class="space-y-1">
                ...
                >
                  ...
                  <span class="text-emerald-400">- ₱{{ Number(item.unitCost || item.costPerUnit || item.price || 0).toLocaleString() }}</span>
                </p>
        ...
        <div class="bg-slate-800 rounded-xl p-8 max-w-2xl w-full mx-auto my-6 border border-slate-700 max-h-[90vh] overflow-y-auto">
          ...
          <form @submit.prevent="addSupplier" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-2 bg-slate-700 rounded-lg border border-slate-600 p-3">
                  <label v-for="cat in categoryOptions" :key="cat" class="flex items-center gap-2 text-slate-200 text-sm">
                    <input
                      ...
                      class="h-4 w-4 rounded border-slate-500 bg-slate-800 text-purple-500 focus:ring-purple-500"
                    />
                    <span>{{ cat }}</span>
                  </label>
                </div>
              ...
              <div class="space-y-2">
                <div v-for="(item, index) in newSupplier.offeredItems" :key="`offer-${index}`" class="grid grid-cols-12 gap-2">
                  <input
                    ...
                    class="col-span-4 bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 focus:border-purple-500 focus:outline-none"
                    @input="markOfferedTouched(index, 'name')"
                    @blur="markOfferedTouched(index, 'name')"
                  />
                  <select
                    v-model="item.category"
                    class="col-span-3 bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 focus:border-purple-500 focus:outline-none"
                  ...
                  <div class="col-span-3 relative">
                    <span class="absolute left-2 top-1/2 -translate-y-1/2 text-slate-300 text-xs">₱</span>
                    <input
                      ...
                      class="w-full bg-slate-700 text-white pl-6 pr-2 py-2 rounded-lg border border-slate-600 focus:border-purple-500 focus:outline-none"
            ...
            <div class="flex justify-end gap-3 mt-6">
              ...
              <button type="submit" :disabled="saving" class="px-6 py-2 rounded-lg bg-purple-500 hover:bg-purple-600 text-white transition-colors disabled:opacity-50">
        ...
        <div class="bg-slate-800 rounded-xl p-8 max-w-2xl w-full mx-auto my-6 border border-slate-700 max-h-[90vh] overflow-y-auto">
          ...
          <form @submit.prevent="saveSupplierEdit" class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-slate-400 text-sm mb-2">Supplier Name</label>
                <input v-model="editSupplier.name" type="text" required class="w-full bg-slate-700 text-white px-4 py-2 rounded-lg border border-slate-600 focus:border-purple-500 focus:outline-none" />
              </div>
              <div class="col-span-2">
                <label class="block text-slate-400 text-sm mb-2">Categories (Multiple)</label>
                <div class="grid grid-cols-2 gap-2 bg-slate-700 rounded-lg border border-slate-600 p-3">
              ...
              <div>
                <label class="block text-slate-400 text-sm mb-2">Contact Person</label>
                <input v-model="editSupplier.contact" type="text" required class="w-full bg-slate-700 text-white px-4 py-2 rounded-lg border border-slate-600 focus:border-purple-500 focus:outline-none" />
              </div>
              <div>
                <label class="block text-slate-400 text-sm mb-2">Email</label>
                <input v-model="editSupplier.email" type="email" required class="w-full bg-slate-700 text-white px-4 py-2 rounded-lg border border-slate-600 focus:border-purple-500 focus:outline-none" />
              </div>
              <div>
                <label class="block text-slate-400 text-sm mb-2">Phone</label>
                <div class="relative">
                  ...
                  <input
                    ...
                    type="tel"
                    required
                    class="w-full bg-slate-700 text-white pl-12 pr-4 py-2 rounded-lg border border-slate-600 focus:border-purple-500 focus:outline-none"
                    inputmode="numeric"
                  />
                </div>
              </div>
              <div>
                <label class="block text-slate-400 text-sm mb-2">Status</label>
                <select v-model="editSupplier.status" required class="w-full bg-slate-700 text-white px-4 py-2 rounded-lg border border-slate-600 focus:border-purple-500 focus:outline-none">
                  <option value="Active">Active</option>
                  <option value="Inactive">Inactive</option>
                </select>
              </div>
            ...
            <div>
              <label class="block text-slate-400 text-sm mb-2">Address</label>
              <textarea v-model="editSupplier.address" rows="3" class="w-full bg-slate-700 text-white px-4 py-2 rounded-lg border border-slate-600 focus:border-purple-500 focus:outline-none"></textarea>
            </div>
            <div class="flex justify-end gap-3 mt-6">
...
export default {
  ...
  setup() {
    ...
    const inputClass = (hasError) => [
      ...
      hasError ? 'border-red-500 focus:border-red-500' : 'border-slate-600 focus:border-purple-500'
    ]
