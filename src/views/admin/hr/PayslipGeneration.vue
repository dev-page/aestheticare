<template>
  <div class="flex module-theme bg-slate-900 min-h-screen">
    ...
    <main class="flex-1 p-4 md:p-8">
      ...
      <div v-if="loading" class="bg-slate-800 rounded-xl p-6 border border-slate-700 text-slate-300">
        Loading payslip...
      </div>
  ...
  setup() {
    ...
    const loadStaff = async () => {
      ...
      const candidates = snapshot.docs
        ...
        .filter((staff) => staff.id !== currentUserId.value && staff.role !== 'HR' && !staff.archived)
        .map((staff) => ({
          id: staff.id,
