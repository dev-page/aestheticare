    ...
    <main class="flex-1 flex items-center justify-center p-8">
      <div class="bg-slate-800 rounded-xl p-6 border border-slate-700 w-full max-w-3xl">
        ...
        <form @submit.prevent="saveCustomerProfile" class="space-y-4">
          <div>
            <label class="block text-slate-400 text-sm mb-1">First Name</label>
            <input v-model="customer.firstName" type="text" class="w-full rounded-lg p-3 bg-slate-700 border border-slate-600 text-white" />
          </div>

          <div>
            <label class="block text-slate-400 text-sm mb-1">Last Name</label>
            <input v-model="customer.lastName" type="text" class="w-full rounded-lg p-3 bg-slate-700 border border-slate-600 text-white" />
          </div>

          <div>
            <label class="block text-slate-400 text-sm mb-1">Email</label>
            <input v-model="customer.email" type="email" class="w-full rounded-lg p-3 bg-slate-700 border border-slate-600 text-white" />
          </div>

          <div>
            <label class="block text-slate-400 text-sm mb-1">Phone Number</label>
            <input v-model="customer.contactNumber" type="tel" class="w-full rounded-lg p-3 bg-slate-700 border border-slate-600 text-white" />
          </div>

          <div>
            <label class="block text-slate-400 text-sm mb-1">Address</label>
            <textarea v-model="customer.address" rows="3" class="w-full rounded-lg p-3 bg-slate-700 border border-slate-600 text-white resize-none"></textarea>
          </div>

          <div>
            <label class="block text-slate-400 text-sm mb-1">Bio</label>
            <textarea v-model="customer.bio" rows="4" class="w-full rounded-lg p-3 bg-slate-700 border border-slate-600 text-white resize-none"></textarea>
          </div>

          <button type="submit" class="w-full bg-purple-600 hover:bg-purple-500 text-white font-semibold py-3 rounded-lg transition-colors">
            Save Changes
          </button>
        </form>
      </div>
